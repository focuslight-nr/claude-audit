...
Claude Code / Claude Desktop のローカル設定を読み取り専用で監査する macOS / Windows 用 CLI ツールです。
...
- Windows 10/11 + Windows PowerShell 5.1 以降、または PowerShell 7 以降
- macOS + zsh（macOS 標準）
- macOS 版のみ `jq` 推奨（未導入の場合 JSON 設定の深掘り解析がスキップされます）
